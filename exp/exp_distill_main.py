...
from exp.exp_basic_dis import Exp_Basic
...
class Exp_Main(Exp_Basic):
    ...
    def _build_teacher_model(self):
        ...
        model = model_dict[self.args.teacher_model].Model(self.args).float()
    ...
    def train(self, setting):
        ...
        state_dict = torch.load('pretained_file/checkpoint_lora_96_TSF.pth')
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if any(substr in self.args.model for substr in {'Linear', 'TST', 'SparseTSF'}):
                            ...
                            if self.args.output_attention:
                                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                                teacher_outputs = self.teacher_model(batch_x,batch_x_mark,dec_inp,batch_y_mark)[0]
                            else:
                                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                                teacher_outputs = self.teacher_model(batch_x,batch_x_mark,dec_inp,batch_y_mark)

                        f_dim = -1 if self.args.features == 'MS' else 0
                        outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                        loss = criterion(outputs, batch_y)
                        dis_loss = criterion(outputs,teacher_outputs)
                        total_loss = loss
